...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  async function createOrganization() {
    ...
    if (body?.whatsapp_broadcast?.length > 0) {
      const create_whatsapp_broadcast = body.whatsapp_broadcast;
      body.whatsapp_broadcast = {
        create: create_whatsapp_broadcast,
      };
    } else {
      delete body.whatsapp_broadcast;
    }
