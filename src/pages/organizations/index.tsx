...
export function OrganizationListPage(props: OrganizationListPageProps) {
  ...
  const fetcher = useCallback(
    async () =>
      getOrganizations({
        relations: ['user', 'course.count', 'faq.count', 'newsletter.count', 'whatsapp_broadcast.count'],
  ...
  const columns: ColumnType[] = [
    ...
    hasAccess('course', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ...
      : null,
    hasAccess('faq', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          id: 'faq',
          header: 'Faq',
          accessorKey: 'faq',
          cell: ({ row: { original: record } }: any) => record?._count?.faq,
        }
      : null,
    hasAccess('newsletter', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          ...
          cell: ({ row: { original: record } }: any) => record?._count?.newsletter,
        }
      : null,
    hasAccess('whatsapp_broadcast', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          id: 'whatsapp_broadcast',
          header: 'Whatsapp Broadcast',
          accessorKey: 'whatsapp_broadcast',
          cell: ({ row: { original: record } }: any) => record?._count?.whatsapp_broadcast,
        }
      : null,
