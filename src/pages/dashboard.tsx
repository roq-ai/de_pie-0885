...
import { getWhatsappBroadcasts } from 'apiSdk/whatsapp-broadcasts';
import { WhatsappBroadcastInterface } from 'interfaces/whatsapp-broadcast';
import { getCourses } from 'apiSdk/courses';
import { CourseInterface } from 'interfaces/course';
import { getNewsletters } from 'apiSdk/newsletters';
import { NewsletterInterface } from 'interfaces/newsletter';

import { FaqListPage as ShowCaseList } from 'pages/faqs';

function DashboardPage() {
  const { session } = useSession();

  const whatsappBroadcastStat = useSWR<PaginatedInterface<WhatsappBroadcastInterface>>(
    () => `/whatsapp-broadcasts`,
    () => getWhatsappBroadcasts({ limit: 1, offset: 0 }),
  );
  ...
  const newsletterStat = useSWR<PaginatedInterface<NewsletterInterface>>(
    () => `/newsletters`,
    () => getNewsletters({ limit: 1, offset: 0 }),
  );

  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      title: 'Whatsapp Broadcasts',
      result: whatsappBroadcastStat?.data?.totalCount,
      isLoading: whatsappBroadcastStat.isLoading,
    ...
    {
      title: 'Newsletters',
      result: newsletterStat?.data?.totalCount,
      isLoading: newsletterStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
          ...
          <Stack direction="column" spacing={8} flex={{ base: 1, md: 7 }}>
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                ...
                <NextLink href={`faqs`} passHref legacyBehavior>
                  <Button
